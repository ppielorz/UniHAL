                                        ...
                                        const UniHAL_gpio_t* const cs, const UniHAL_gpio_t* const rst, const UniHAL_gpio_t* const dc)
{
    ...
    /*writecmd(display, 0xAE);  // Display Off
        ...
        writecmd(display, 0x01);  // Horizontal Addressing Mode

        ssd1306_setContrast(display, 0xFF);

        writecmd(display, 0xA0);
        ...
        writecmd(display, 0x02);  // Alternative COM Pin---See IC Spec page 34
...
/******************************************************************************
 Local Functions
 *****************************************************************************/
static uint8_t commands[1024];
static size_t commandCounter = 0U;
static SSD1306_status_t sendCommand(const SSD1306_t* const display, const uint8_t command)
{
    if(commandCounter < sizeof(commands))
    {
        commands[commandCounter] = command;
        commandCounter++;
    }
    CHECK_AND_RETURN_STATUS(display != NULL, SSD1306_status_nullPointer);
